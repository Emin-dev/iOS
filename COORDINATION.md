...
- Emin: drivers and the Sea Breeze team are salaried staff, so their screens show no money. Removed fares, earnings (ST13 deleted), revenue, discounts and deposit amounts in all six versions; replaced with Jobs today, Online today, Trips today, Carts returned, Change cart. Admin keeps its Today totals. Page renamed by Emin to "Admin & Staff".
- Emin: staff action tiles use orange tones only. Accept / Approve = brand/solid orange with white text; Decline / Block = brand/tint with gold text; big status tiles (Start ride, End ride, End trip, Close the day) now have white text. All six versions.
- Emin: staff and admin screens have no tab bar (all six versions). Added: map with pickup pin on the new-job card (ST11); lock-screen push for new job (ST11p) and golf request (ST21p) with Accept / Decline / Open; Live Activity + Dynamic Island for the active job (ST12L). Messages tiles point to C01 Chats. Red kept only for danger states. Sign-in stays: hidden Admin row + phone code.
- New staff screens in all six versions: driver edge cases (ST10c shift ending, ST10d no internet, ST12c rider not here / no-show, ST12d rider cancelled), Sea Breeze ST23b Extend and ST23c Change cart, admin ST31a decline reason, ST31b photos, ST31c documents, ST33a turn off car, ST34a block user. Prototype links wired inside each version.
